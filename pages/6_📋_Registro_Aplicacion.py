...
# --- Obtener tareas completadas localmente ---
try:
...
# Filtrar tareas que están "Programada" Y que no han sido completadas localmente
    ...
    for index, tarea in tareas_pendientes.iterrows():
        with st.container(border=True):
            ...
            if st.button("✅ Marcar como Terminado", key=f"complete_{tarea['ID_Tarea']}"):
                # Guardar el ID de la tarea en el almacenamiento local
...
# --- SECCIÓN 2: SINCRONIZACIÓN DE TAREAS COMPLETADAS ---
...
if tareas_completadas_local:
    ...
    if st.button("Sincronizar Ahora"):
        with st.spinner("Sincronizando..."):
            try:
                # Recargar los archivos para tener la última versión
                ...
                st.error(f"Error de conexión o datos. No se pudo sincronizar. Sus datos locales están a salvo. Detalles: {e}")
else:
    st.info("✅ Todas las aplicaciones completadas están sincronizadas.")

st.divider()

# --- SECCIÓN 3: PROGRAMAR NUEVA APLICACIÓN (Requiere Conexión) ---
